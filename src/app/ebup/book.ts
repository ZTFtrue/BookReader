...
class Book {
	...
	constructor(url, options) {
		 ...
		 * @memberof Book
		 */
		this.locations = new Locations(this.spine, this.load.bind(this),null);
		...
		if (url) {
			this.open(url, this.settings.openAs).catch((error) => {
				...
				this.emitter.emit(EVENTS.BOOK.OPEN_FAILED, err);
			});
		}
	}
	...
	open(input, what) {
		...
		if (type === INPUT_TYPE.BINARY) {
			this.archived = true;
			this.url = new Url("/", "");
			opening = this.openEpub(input,null);
	...
	openContainer(url) {
		return this.load(url,null)
			.then((xml) => {
				...
				return this.resolve(this.container.packagePath,null);
			});
	}
	...
	openManifest(url) {
		...
		return this.load(url,null)
			.then((json) => {
				this.packaging = new Packaging(null);
	 ...
	 */
	load(path,option) {
		var resolved = this.resolve(path,null);
		if (this.archived) {
	...
	unpack(packaging) {
		...
		if (this.packaging.metadata.layout === "") {
			...
			this.load(this.url.resolve(IBOOKS_DISPLAY_OPTIONS_PATH),null).then((xml) => {
				this.displayOptions = new DisplayOptions(xml);
				this.loading.displayOptions.resolve(this.displayOptions);
			}).catch((err) => {
				this.displayOptions = new DisplayOptions(null);
				this.loading.displayOptions.resolve(this.displayOptions);
			});
		} else {
			this.displayOptions = new DisplayOptions(null);
		...
		if (this.packaging.coverPath) {
			this.cover = this.resolve(this.packaging.coverPath,null);
		}
		// Resolve promises
	...
	loadNavigation(packaging) {
		...
		if (!navPath) {
			return new Promise((resolve, reject) => {
				...
				this.pageList = new PageList(null);
	 ...
	 */
	getRange(cfiRange) {
		var cfi = new EpubCFI(cfiRange,null,null);
		...
		return item.load(_request).then(function (contents) {
			var range = cfi.toRange(item.document,null);
			return range;
		});
	}
