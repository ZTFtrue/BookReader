...
class Contents {
	...
	locationOf(target, ignoreClass) {
		...
		if (this.epubcfi.isCfiString(target)) {
			let range = new EpubCFI(target, null, null).toRange(this.document, ignoreClass);
	...
	map(layout) {
		var map = new Mapping(layout, null, null);
		return map.section();
	}
	 ...
	 */
	size(width, height) {
		var viewport = { scale: 1.0, scalable: "no", height: null, width: null };
		...
		if (width >= 0) {
			...
			this.css("padding", "0 " + (width / 12) + "px", null);
		}
		...
		this.css("margin", "0", null);
		this.css("box-sizing", "border-box", null);
	...
	columns(width, height, columnWidth, gap, dir) {
		...
		this.css("overflow-y", "hidden", false);
		this.css("margin", "0", true);

		if (axis === "vertical") {
			this.css("padding-top", (gap / 2) + "px", true);
			this.css("padding-bottom", (gap / 2) + "px", true);
			this.css("padding-left", "20px", false);
			this.css("padding-right", "20px", false);
			this.css(COLUMN_AXIS, "vertical", false);
		} else {
			this.css("padding-top", "20px", false);
			this.css("padding-bottom", "20px", false);
			this.css("padding-left", (gap / 2) + "px", true);
			this.css("padding-right", (gap / 2) + "px", true);
			this.css(COLUMN_AXIS, "horizontal", false);
		}

		this.css("box-sizing", "border-box", false);
		this.css("max-width", "inherit", false);

		this.css(COLUMN_FILL, "auto", false);

		this.css(COLUMN_GAP, gap + "px", false);
		this.css(COLUMN_WIDTH, columnWidth + "px", false);

		// Fix glyph clipping in WebKit
		// https://github.com/futurepress/epub.js/issues/983
		this.css("-webkit-line-box-contain", "block glyphs replaced", false);
	}
	...
	scaler(scale, offsetX, offsetY) {
		...
		this.css("transform-origin", "top left", null);

		if (offsetX >= 0 || offsetY >= 0) {
			translateStr = " translate(" + (offsetX || 0) + "px, " + (offsetY || 0) + "px )";
		}

		this.css("transform", scaleStr + translateStr, null);
	}
	...
	fit(width, height, section) {
		var viewport = this.viewport(null);
		...
		this.css("background-size", viewportWidth * scale + "px " + viewportHeight * scale + "px", false);

		this.css("background-color", "transparent", false);
		if (section && section.properties.includes("page-spread-left")) {
			// set margin since scale is weird
			var marginLeft = width - (viewportWidth * scale);
			this.css("margin-left", marginLeft + "px", false);
		}
	}
	...
	mapPage(cfiBase, layout, start, end, dev) {
		var mapping = new Mapping(layout, dev, null);
	...
	epubReadingSystem(name, version) {
		...
		// 	name: name,
		// 	version: version,
		// 	layoutStyle: this.layoutStyle(null),
