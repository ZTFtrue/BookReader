...
class DefaultViewManager {
	...
	afterDisplayed(view) {
		this.emitter.emit(EVENTS.MANAGERS.ADDED, view);
	}
	...
	onScroll() {
		...
		if (!this.ignore) {
			this.emitter.emit(EVENTS.MANAGERS.SCROLL, {
				top: scrollTop,
				left: scrollLeft
			});
	...
	applyLayout(layout) {
		...
		if (this.views && this.views.length > 0 && this.layout.name === "pre-paginated") {
			this.display(this.views.first().section, null);
		}
	...
	updateFlow(flow, defaultScrolledOverflow = "auto") {
		...
		if (flow === "scrolled-doc" ||
			...
			flow === "scrolled") {
			this.updateAxis("vertical", null);
		} else {
			this.updateAxis("horizontal", null);
		}
