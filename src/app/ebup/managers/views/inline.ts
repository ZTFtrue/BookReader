...
class InlineView {
	...
	resize(width, height) {
		...
		this.emitter.emit(EVENTS.VIEWS.RESIZED, size);
	...
	load(contents) {
		...
		var doc = parse(contents, "text/html",null);
		...
		*/
		...
		this.contents = new Contents(this.document, this.frame,null,null);
	...
	display(request) {
		...
		if (!this.displayed) {
			...
			this.render(request, null).then(function () {
	...
	show() {
		...
		this.emitter.emit(EVENTS.VIEWS.SHOWN, this);
	}
	...
	hide() {
		...
		this.emitter.emit(EVENTS.VIEWS.HIDDEN, this);
	}
