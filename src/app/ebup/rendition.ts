...
class Rendition {
	...
	_display(target) {
		...
		this.manager.display(section, target)
			.then(() => {
				 ...
				 * @memberof Rendition
				 */
				this.emitter.emit(EVENTS.RENDITION.DISPLAYED, section);
				this.reportLocation();
			}, (err) => {
				/**
				 ...
				 * @memberof Rendition
				 */
				this.emitter.emit(EVENTS.RENDITION.DISPLAY_ERROR, err);
			});
	...
	afterRemoved(view) {
		this.hooks.unloaded.trigger(view, this).then(() => {
			 ...
			 * @memberof Rendition
			 */
			this.emitter.emit(EVENTS.RENDITION.REMOVED, view.section, view);
		});
	}
	...
	onOrientationChange(orientation) {
		 ...
		 * @memberof Rendition
		 */
		this.emitter.emit(EVENTS.RENDITION.ORIENTATION_CHANGE, orientation);
	}
	...
	layout(settings) {
		if (settings) {
			...
			this._layout.on(EVENTS.LAYOUT.UPDATED, (props, changed) => {
				this.emitter.emit(EVENTS.RENDITION.LAYOUT, props, changed);
			})
		}
	...
	triggerViewEvent(e, contents) {
		this.emitter.emit(e.type, e, contents);
	}
	...
	triggerSelectedEvent(cfirange, contents) {
		 ...
		 * @memberof Rendition
		 */
		this.emitter.emit(EVENTS.RENDITION.SELECTED, cfirange, contents);
	}
	...
	triggerMarkEvent(cfiRange, data, contents) {
		 ...
		 * @memberof Rendition
		 */
		this.emitter.emit(EVENTS.RENDITION.MARK_CLICKED, cfiRange, data, contents);
	}
	...
	getRange(cfi, ignoreClass) {
		var _cfi = new EpubCFI(cfi, null, null);
